...
const CodeEditor: React.FC<CodeEditorProps> = ({
  ...
  const handleBlur = useCallback(() => {
    if (!readOnly && formatOnBlur && value.trim()) {
      const formattedJSON = formatJSON(value);
      if (formattedJSON !== value) {
        onChange(formattedJSON);
      }
    }
  }, [readOnly, formatOnBlur, value, formatJSON, onChange]);

  const handleKeyDown = useCallback(
    (event: React.KeyboardEvent) => {
      if (!readOnly && formatOnSave && (event.ctrlKey || event.metaKey) && event.key === 's') {
    ...
    [readOnly, formatOnSave, value, formatJSON, onChange]
  );
  ...
  return (
    ...
    >
      <CodeMirror
        ...
        onChange={onChange}
        theme="dark"
        className={`text-sm w-full border border-gray-700 rounded-md overflow-auto ${className} ${
          error ? 'border-red-700' :  readOnly?'': 'border-green-500'
        }`}
        height={height}
        readOnly={readOnly} // Pass readOnly to CodeMirror
        editable={!readOnly} // Ensure the editor is non-editable
      />
    </div>
  );
};
